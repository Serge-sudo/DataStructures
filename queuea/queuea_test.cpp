...
TEST_CASE("STRESS") {
    ...
    std::vector<QueueA<int> *> queues{&q1, &q2};
    for (auto q : queues) {
...
TEST_CASE("QueueA copy assignment operator") {
    QueueA<int> q1;
    q1.push(1);
    q1.push(2);
    q1.push(3);
...
TEST_CASE("QueueA move assignment operator") {
    QueueA<int> q1;
    q1.push(1);
    q1.push(2);
    q1.push(3);
...
TEST_CASE("queuelt performance test") {
    const int COUNT = 100;
    const int n = 100000;
    std::ofstream file("statQueueA.txt");
